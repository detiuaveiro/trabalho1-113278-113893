...
int main(int ac, char* av[]) {
  ...
  while (k < ac)
  {
    ...
    else if (strcmp(av[k], "crop") == 0)
    {
      ...
      if (sscanf(av[k], "%d,%d,%d,%d", &x, &y, &w, &h) != 4)
      {
        err = 5;
        break;
      }
      if (!ImageValidRect(img[n - 1], x, y, w, h))
      {
        err = 5;
        break;
      } // precondition check!
    ...
    else if (strcmp(av[k], "blend") == 0)
    {
      ...
      double alpha;
      if (sscanf(av[k], "%d,%d,%lf", &x, &y, &alpha) != 3)
      {
        err = 5;
        break;
      }
